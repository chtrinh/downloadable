...
module Spree::Checkout
  ...
  def checkout
    ...
    @order.line_items.each do |item|  #Loop to make sure all products are downloadable.
      if item.product.product_downloads.empty? 
    ...
    if @isDownloadable
      @checkout_steps = %w{billing payment confirmation} #skip shipping related steps
    else
    ...
    if request.put?                           
      ...
      if @isDownloadable
        @order.ship_address.update_attributes(@order.bill_address.attributes)
      end
      @order.update_totals

...
class Product < ActiveRecord::Base
  ZIP_BASE = "downloadable/products/"
  ZIP_ROOT = "#{RAILS_ROOT}/public/"
  ZIP_LOCATION = ZIP_ROOT + ZIP_BASE
  ...
  def bundlefile
    self.bundle_filename.nil? ? false : (ZIP_BASE + self.bundle_filename)
  end
  
  # create a zipped archive file of all product_downloads
  def bundle(name = self.permalink)
    unless self.product_downloads.empty?
       bundle_filename = ZIP_LOCATION + "#{name}.zip"
       ...
       self.bundle_filename = "#{name}.zip"

       # open or create the zip file
       Zip::ZipFile.open(bundle_filename, Zip::ZipFile::CREATE) {
         |zipfile|
         # collect the downlodables
         self.product_downloads.collect {
           |download|
             if download.enabled
               # add each download to the archive
               zipfile.add(download.filename, download.downloadable.path)
             end
           }
       }
